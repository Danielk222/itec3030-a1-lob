...
/**
 * Represents a stock exchange that manages securities, accounts, orders, and
 * trades.
 */
public class Exchange {
	...
	public boolean validateOrder(IOrder o) {
		...
		if (accounts.getTraderByID(o.getTraderID()) == null) {
		...
		int pos = accounts.getTraderAccount.getPosition(o.getTicker());
		//Get the balance the trader has with the exchange
		long bal = accounts.getTraderAccount.getBalance();
	...
	public void submitOrder(IOrder o, long time) {
		...
		for (ITrade t:oOutcome.getResultingTrades()) {
			...
			long buyerPayFee = t.getBuyerFee();
			//Apply the above fee to the account balance of the buyer 			
			accounts.getTraderAccount(accounts.getTraderByID(t.getBuyerID())).applyFee(buyerPayFee);
			//Apply the trade payment to the account balance of the buyer (they spent money)
			accounts.getTraderAccount(accounts.getTraderByID(t.getBuyerID())).updateBalance(-t.getTradePayment());
			//Add the bought stocks to the position of the buyer
			accounts.getTraderAccount(accounts.getTraderByID(t.getBuyerID())).updatePosition(t.getTicker(), t.getQuantity();
			
			//Update balances for Seller
			
			//Get the fee that the seller is supposed to pay
			long sellerPayFee = t.getSellerFee;
			//Apply the above fee to the account balance of the seller
			accounts.getTraderAccount(accounts.getTraderByID(t.getSellerID())).applyFee(SellerPayFee);
			//Apply the trade payment to the account balance of the seller (they earned money)
            accounts.getTraderAccount(accounts.getTraderByID(t.getSellerID())).updateBalance(t.getTradePayment()));			//Deduct the sold stocks from the position of the seller
            accounts.getTraderAccount(accounts.getTraderByID(t.getSellerID())).updatePosition(t.getTicker(), -t.getQuantity());			
